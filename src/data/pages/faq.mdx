---
title: "FAQ — VERITIR"
description: "Frequently asked questions about VERITIR pilots, data readiness, governance, and pricing."
---
...
<section class="py-20 md:py-28">
  <div class="mx-auto max-w-3xl px-6">
    <h1 class="text-4xl font-bold tracking-tight md:text-5xl">Frequently asked questions</h1>
    <p class="mt-4 text-lg text-neutral-400">Common questions about VERITIR pilots, data readiness, governance, and pricing.</p>

    <div class="mt-16 space-y-12">
      {faqs.map((faq) => (
        <div>
          <h2 class="text-xl font-semibold text-white">{faq.q}</h2>
          <p class="mt-3 text-neutral-300 leading-relaxed">{faq.a}</p>
          {faq.bullets && (
            <ul class="mt-3 space-y-2 pl-1">
              {faq.bullets.map((b) => (
                <li class="flex items-start gap-3">
                  <span class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent" />
                  <span class="text-neutral-300">{b}</span>
                </li>
              ))}
            </ul>
          )}
    ...
    <div class="mt-20 border-t border-border pt-12">
      <p class="text-neutral-400">Have a question not covered here?{" "}<a href="mailto:tim@veritir.com" class="text-accent hover:underline">Reach out directly</a>.</p>
    </div>
  </div>
</section>
